...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">投递简历管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>投递简历列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th>个人简历</th>
						<th onclick="sort('toudishijian')">投递时间<i id="toudishijianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var toudishijianCell  = document.createElement('td');

						  toudishijianCell.innerHTML = item.toudishijian;


			row.appendChild(toudishijianCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "面试邀请"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'mianshiyaoqing','是','','')");
			if(crossBtnControl('面试邀请') == true){
		...
		function sh(){
			...
			httpJson("toudijianli/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("toudijianli/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"toudijianli");
