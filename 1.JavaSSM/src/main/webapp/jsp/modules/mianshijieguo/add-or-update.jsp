...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑面试结果</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>面试结果管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑面试结果</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>标题</label>
					<input name="biaoti" id="biaoti" name="biaoti" class="form-control" placeholder="请输入标题">
				</div>
				<div class="form-item">
					<label>结果</label>
					<select id="jieguoSelect" name="jieguo" class="form-control">
							<option value="">请选择结果</option>
							<option class="jieguoOption" value="通过">
								通过
							</option>
							<option class="jieguoOption" value="失败">
								失败
							</option>
					</select>	
				</div>	
				<div class="form-item">
					<label>结果说明</label>
					<textarea  name="jieguoshuoming" class="form-control textarea" id="jieguoshuoming" rows="3"></textarea>
				</div>
				<div class="form-item">
					<label>时间</label>
					<input id="shijian-input" class="date" name="shijian" size="20" type="text" readonly placeholder="请选择时间">
				</div>
				<div class="form-item">
					<label>企业账号</label>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "mianshijieguo/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("mianshijieguo/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("mianshijieguo/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("mianshijieguo/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#shijian-input").val(getCurDateTime());
			$("#shijian-input").datetimepicker({ 
				autoclose: true,
		...
		function calculationSE(colName){
			//单列求和接口
			http("mianshijieguo"+colName,"GET",{
		...
		function dataBind(){
			...
			var jieguoOptions = document.getElementsByClassName("jieguoOption");
			for(var jieguoCount = 0; jieguoCount < jieguoOptions.length;jieguoCount++){
				if(jieguoOptions[jieguoCount].getAttribute('value') == ruleForm.jieguo){
					jieguoOptions[jieguoCount].setAttribute('selected','selected');
				}
			}
			$("#jieguoshuoming").val(ruleForm.jieguoshuoming);
			$("#shijian-input").val(ruleForm.shijian);
			...
			$("#crossuserid").val(ruleForm.crossuserid);	
			$("#crossrefid").val(ruleForm.crossrefid);	
		}		
		//图片显示
		function showImg(){
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jieguo != null && res.data.jieguo != ''){

						var jieguoOptions = document.getElementsByClassName("jieguoOption");
						for(var jieguoCount = 0; jieguoCount < jieguoOptions.length;jieguoCount++){
							if(jieguoOptions[jieguoCount].getAttribute('value') == res.data.jieguo){
								jieguoOptions[jieguoCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.jieguoshuoming != null && res.data.jieguoshuoming != ''){

						$("#jieguoshuoming").val(res.data.jieguoshuoming);
					}
					if(res.data != null && res.data != '' && res.data.shijian != null && res.data.shijian != ''){

						$("#shijian-input").val(res.data.shijian);
						$('#shijianFileName').val(res.data.shijian)
					}
					...
					if(res.data != null && res.data != '' && res.data.crossuserid != null && res.data.crossuserid != ''){

						$("#crossuserid").val(res.data.crossuserid);
						$("#crossuserid").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.crossrefid != null && res.data.crossrefid != ''){

						$("#crossrefid").val(res.data.crossrefid);
						$("#crossrefid").attr('readonly','readonly');
					}
				}
			});  
            }
