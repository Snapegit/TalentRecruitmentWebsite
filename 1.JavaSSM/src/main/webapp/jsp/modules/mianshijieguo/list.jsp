...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">面试结果管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>面试结果列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				...
				<div class="form-item">
					<label>结果</label>
					<select name="jieguo" id="jieguoSelect" class="form-control form-control-sm" aria-controls="tableId">
						<option value=" "></option>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('jieguo')">结果<i id="jieguoIcon" class="fa fa-sort"></i></th>
						<th>结果说明</th>
	...
	<script>
		...
		function search(){
			...
			var jieguoIndex = document.getElementById("jieguoSelect").selectedIndex;
			var jieguoOptions = document.getElementById("jieguoSelect").options;
			var jieguoSelectedOption = jieguoOptions[jieguoIndex]
			if(jieguoSelectedOption.text != null && jieguoSelectedOption.text != ''){
				searchForm.jieguo = jieguoSelectedOption.text;
			}
            pageIndex=1;
			getDataList();
		}		
		...
		function setDataRow(item,number){
			...
			var jieguoCell  = document.createElement('td');

						 jieguoCell.innerHTML = item.jieguo;

			row.appendChild(jieguoCell);
			var jieguoshuomingCell  = document.createElement('td');

						  if(item.jieguoshuoming != null && item.jieguoshuoming != "" && item.jieguoshuoming.length >= 11){
			  	jieguoshuomingCell.innerHTML = item.jieguoshuoming.substring(0, 10) + "..."; 
			  }else{
			  	jieguoshuomingCell.innerHTML = item.jieguoshuoming;
			  }


			row.appendChild(jieguoshuomingCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("mianshijieguo/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"mianshijieguo");
