...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">面试邀请管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>面试邀请列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th>头像</th>
						<th onclick="sort('mianshididian')">面试地点<i id="mianshididianIcon" class="fa fa-sort"></i></th>
						<th>准备资料</th>
						<th onclick="sort('mianshishijian')">面试时间<i id="mianshishijianIcon" class="fa fa-sort"></i></th>
						<th>操作</th>
					</tr>
				</thead>
				<tbody></tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var mianshididianCell  = document.createElement('td');

						 mianshididianCell.innerHTML = item.mianshididian;

			row.appendChild(mianshididianCell);
			var zhunbeiziliaoCell  = document.createElement('td');

						  if(item.zhunbeiziliao != null && item.zhunbeiziliao != "" && item.zhunbeiziliao.length >= 11){
			  	zhunbeiziliaoCell.innerHTML = item.zhunbeiziliao.substring(0, 10) + "..."; 
			  }else{
			  	zhunbeiziliaoCell.innerHTML = item.zhunbeiziliao;
			  }


			row.appendChild(zhunbeiziliaoCell);
			var mianshishijianCell  = document.createElement('td');

						  mianshishijianCell.innerHTML = item.mianshishijian;


			row.appendChild(mianshishijianCell);

			//每行按钮
			var btnGroup = document.createElement('td');

			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "参加面试"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'canjiamianshi','','[1]','请勿重复操作!')");
			if(crossBtnControl('参加面试') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("mianshiyaoqing/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"mianshiyaoqing");
