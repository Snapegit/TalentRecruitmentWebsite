...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">参加面试管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>参加面试列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				...
				<div class="form-item">
					<label>面试地点</label>
					<input type="text" id="mianshididianSearch" class="form-control form-control-sm" placeholder="请输入面试地点" aria-controls="tableId">
				</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('canjiashijian')">参加时间<i id="canjiashijianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#mianshididianSearch').val() != null && $('#mianshididianSearch').val() != ''){
				searchForm.mianshididian ="%" +  $('#mianshididianSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("canjiamianshi/page","GET",{
				...
				mianshididian : searchForm.mianshididian,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var canjiashijianCell  = document.createElement('td');

						  canjiashijianCell.innerHTML = item.canjiashijian;


			row.appendChild(canjiashijianCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "面试结果"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'mianshijieguo','','[1]','请勿重复操作!')");
			if(crossBtnControl('面试结果') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("canjiamianshi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"canjiamianshi");
